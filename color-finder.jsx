...
// Product thumb: real swatch/bullet photo, zoomed slightly to crop the
// product photographers' white padding. Falls back to a solid color tile.
    ...
    <div style={{
      ...
      {showImg ? (
        ...
        <div style={{
          ...
          boxShadow:`inset 0 2px 8px ${product.hex}70`,
        }} />
      )}
    </div>
  );
          ...
          <tbody>
            ...
            {filtered.map((p, i) => (
              ...
              >
                ...
                {/* Product color swatch derived from CIELAB */}
                <td style={{ padding:'14px 16px' }}>
                  <div style={{
                    width:28, height:28, borderRadius:'50%',
                    background: p.hex,
                    boxShadow:`0 2px 6px ${p.hex}70`,
                    border:'1.5px solid rgba(42,26,20,0.08)',
                  }} title={p.shade} />
                </td>
...
function TweaksPanel({ tweaks, setTweak, onClose }) {
  return (
    <div style={{
      ...
      <label style={{ display:'block', marginBottom:6, color:'var(--text-muted)', fontSize:11, letterSpacing:'0.08em', textTransform:'uppercase' }}>Matches Shown</label>
      <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:18 }}>
        <input type="range" min={1} max={5} value={tweaks.matchCount}
          onChange={e => setTweak('matchCount', +e.target.value)}
          style={{ flex:1, accentColor:'var(--blush)' }} />
        <span style={{ width:20, textAlign:'center', color:'var(--espresso)', fontWeight:500 }}>{tweaks.matchCount}</span>
      </div>
...
function HexPicker({ sampledHex, onColor }) {
  ...
  // Keep draft in sync if cleared externally (e.g. tab switch)
  React.useEffect(() => {
  ...
  // Suggestion swatches: a few common starting points
  const QUICK = [
...
function App() {
  ...
  const matches = selectedColor
    ? getClosestColors(
        toneRamp && toneIdx != null && !onAnchor ? toneRamp.ramp[toneIdx].hex : selectedColor.hex,
        tweaks.matchCount * 4
      )
        .filter(p => !selectedColor.sourceKey || `${p.brand}|${p.shade}` !== selectedColor.sourceKey)
        .filter(matchesVibe)
        .slice(0, tweaks.matchCount)
    : [];
      ...
      <header className="app-header" style={{
        ...
        <button
          ...
          style={{
            ...
            color: vibeActive ? 'var(--blush)' : 'var(--espresso)', cursor:'pointer',
            fontFamily:'DM Sans', fontSize:12, fontWeight:500, letterSpacing:'0.06em',
            textTransform:'uppercase', transition:'all 0.15s',
            position:'relative',
          }}
        >
          <span style={{ fontSize:13, color:'var(--blush)' }}>✦</span>
          My Lipstick Vibe
          {vibeActive > 0 && (
            <span style={{
      ...
      <main className="app-main" style={{
        ...
        <div className="palette-col">
          ...
          <div style={{
            ...
            {[{id:'wheel',label:'Color wheel'},{id:'photo',label:'From photo'}, ...(wishlist.length > 0 ? [{id:'list',label:'From My List'}] : [])].map(t => (
              <button key={t.id}
                onClick={() => {
                  setMode(t.id);
